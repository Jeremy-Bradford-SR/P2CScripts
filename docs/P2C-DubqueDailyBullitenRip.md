...
Proxy behavior
- The script downloads a proxy list from `PROXY_LIST_URL` and validates a subset in parallel. Valid proxies are used to try getting the initial ASP.NET_SessionId and later to fetch the JSON data.
...
Running
- Ensure `pyodbc` and `requests` are installed.
- Update MSSQL constants and run: `python3 P2C-DubqueDailyBullitenRip.py`
